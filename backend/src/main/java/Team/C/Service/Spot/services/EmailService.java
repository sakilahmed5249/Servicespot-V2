...
public class EmailService {
    ...
    @Value("${email.from.address:ahmedsakil2015@gmail.com}")
    private String fromAddress;

    @Value("${email.from.name:ServiceSpot Team}")
    ...
    public void sendOTPEmail(String toEmail, String otp, String purpose) {
        try {
            ...
            log.info("OTP email sent successfully to: {} from: {}", toEmail, fromAddress);
        } catch (MessagingException e) {
            log.error("Failed to send OTP email to: {}", toEmail, e);
            throw new RuntimeException("Failed to send OTP email", e);
        } catch (java.io.UnsupportedEncodingException e) {
            log.error("Unsupported encoding for from name: {}", fromName, e);
            throw new RuntimeException("Failed to send OTP email due to encoding issue", e);
        }
    }
